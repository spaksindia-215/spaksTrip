...
// Derive request/response filenames from the collection's step name.
// Mirrors the convention adopted in tbo-results/case-05/...
//   Request  → camelCase + optional variant letter   (e.g. fareRuleRequest, bookARequest)
//   Response → all lowercase, no variant letter      (e.g. fareruleResponse, bookResponse)
// All-caps API names like SSR / PriceRBD are flattened to lowercase / camelCase
// (ssr / priceRBD) — pulled from the first word after "Step Nx — ".
function methodSlugs(stepName) {
  ...
  const variant = m[1] ? m[1].toUpperCase() : "";          // 7a → "A"
  const rest    = m[2].trim();
  const word    = (rest.match(/^[A-Za-z][A-Za-z]*/) || ["step"])[0];

  // Dual-PNR / Special-Return / Calendar OB+IB cases need leg disambiguation,
  // otherwise the OB and IB calls of the same method overwrite each other.
  // OB/IB takes precedence over the Step variant letter when both are present.
  const legM = rest.match(/\b(OB|IB)\b/i);
  const leg  = legM ? legM[1].charAt(0).toUpperCase() + legM[1].slice(1).toLowerCase() : ""; // "Ob" / "Ib"
  const suffix = leg || variant;

  const camelBase = /^[A-Z]+$/.test(word)
    ? word.toLowerCase()
    : word.charAt(0).toLowerCase() + word.slice(1);

  return {
    camel: camelBase + suffix,
    lower: word.toLowerCase() + suffix,
  };
}

// Writes both the request body and the wrapped response. Returns the response
// file path (kept as the primary “saved to ...” reference in step logs).
function saveExchange(caseNum, stepName, requestBody, status, responseText) {
  ...
  if (requestBody !== undefined && requestBody !== null && requestBody !== "") {
    ...
    fs.writeFileSync(path.join(dir, `${camel}Request.json`), out + "\n");
  }

  const resFile = path.join(dir, `${lower}Response.json`);
  const resJson = safeJson(responseText);
  fs.writeFileSync(resFile, JSON.stringify({ status, response: resJson || responseText }, null, 2));
  return resFile;
}

// Backwards-compat shim — older code paths call saveResult(caseNum, name, status, body).
// Delegates to saveExchange with no request body (response-only).
function saveResult(caseNum, stepName, status, body) {
  return saveExchange(caseNum, stepName, undefined, status, body);
}
...
async function runStep(step, caseNum, stepIdx, folderName) {
  ...
  // step is a Ticket call with a Passengers body, POST to /Book instead and
  // fire /Ticket separately afterwards.
  //
  // LCC is the opposite: the supplier REJECTS /Book ("Book not allowed for LCCs.
  // Please do Ticket directly."). So skip promotion when IS_LCC=true and let the
  // Passengers payload hit /Ticket directly. IS_LCC=="" (not yet captured) falls
  // back to promoting, matching prior behaviour for GDS-only flows.
  let promotedFromTicket = false;
  if (/\/Ticket(\?|$)/i.test(url) && /"Passengers"\s*:/.test(body || "")) {
    if (VARS.IS_LCC === "true") {
      console.log(dim("  ↪ LCC detected — POSTing /Ticket directly (no /Book promotion)"));
    } else {
  ...
  // saveExchange writes <method>Request.json + <method>Response.json side by side.
  // If we promoted /Ticket→/Book above, attribute this save to the actual URL hit so
  // the request lands in bookRequest.json (not ticketRequest.json).
  const saveName = promotedFromTicket ? name.replace(/Ticket/gi, "Book") : name;
  const saved    = saveExchange(caseNum, saveName, body, result.status, result.text);
  ...
  // Leg-scoped booking-id capture for dual-PNR cases (Case 03 "Ticket OB" / "Ticket IB").
  // The collection's TICKET_OB_TEST / TICKET_IB_TEST scripts only log ticket numbers
  // and don't write the OB_BOOKING_ID / IB_BOOKING_ID vars that downstream
  // GetBookingDetails OB/IB rely on. Fill them in from the response shape here.
  if (/ticket/i.test(name) && json && result.status === 200 && !(apiErr?.ErrorCode && apiErr.ErrorCode !== 0)) {
    const legM = name.match(/\b(OB|IB)\b/i);
    if (legM) {
      const prefix = legM[1].toUpperCase();
      // /Ticket and /Book share the response shape: BookingId/PNR may sit at
      // Response (LCC direct issue), Response.Response (Book wrapper), or inside FlightItinerary.
      ...
      if (bid !== undefined && bid !== null && bid !== "") {
        VARS[`${prefix}_BOOKING_ID`] = String(bid);
        console.log(green(`  ✓ ${prefix}_BOOKING_ID captured: `) + cyan(String(bid)));
      }
      if (pnr) {
        VARS[`${prefix}_PNR`] = pnr;
        console.log(green(`  ✓ ${prefix}_PNR captured: `) + cyan(pnr));
      }
    }
  }
  ...
  if (promotedFromTicket && result.status === 200 && !(apiErr?.ErrorCode && apiErr.ErrorCode !== 0 && apiErr.ErrorCode !== "0")) {
    ...
    if (VARS.IS_LCC === "false") {
      ...
      if (tRes) {
        // Force this follow-up to land in ticketRequest.json / ticketResponse.json
        // regardless of the original step's name (the step itself was saved as `book*`).
